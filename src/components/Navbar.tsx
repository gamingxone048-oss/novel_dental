       ...
       {isMobileMenuOpen && (
        <div className="md:hidden mt-2 mx-4">
          <div className="bg-white rounded-lg shadow-lg p-4 flex flex-col space-y-2">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="block px-4 py-2 text-gray-700 font-semibold rounded-md hover:bg-gray-100">
                {link.name}
              </a>
            ))}
             <a href="#/contact" className="block text-center mt-2 bg-brand text-white px-4 py-2 rounded-full font-semibold">
